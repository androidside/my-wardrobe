...
Implement social media functionality to allow users to create usernames, manage friends (with approval or direct follow), and share wardrobes with friends. Items are private by default, and only items in shareable wardrobes are visible to friends.

## Requirements

### Core Features
- [ ] Username/Nickname system for user identification
- [ ] Friend system with two modes:
  - [ ] Direct follow (if user allows it)
  - [ ] Friend requests with approval workflow
- [ ] Wardrobe-level privacy controls (mark wardrobes as shareable)
- [ ] View friend's shareable wardrobes and items (read-only)
...
### 3. Friend Search Dialog (New)
**File**: `src/components/FriendSearchDialog.tsx`
...
### 4. Friend Requests Dialog (New)
**File**: `src/components/FriendRequestsDialog.tsx`
...
### 5. Friends Wardrobe View (New)
**File**: `src/components/FriendWardrobeView.tsx`
...
**File**: `src/utils/socialMigration.ts`
...
- [ ] Users can directly follow users who allow it
- [ ] Users can see their friends list
- [ ] Users can mark wardrobes as shareable
- [ ] Users can view friend's shareable wardrobes (read-only)
- [ ] All operations respect privacy settings
- [ ] Firestore security rules are updated
- [ ] All edge cases are handled (duplicate requests, self-follow, etc.)

## Related Files

See detailed implementation plan in: `SOCIAL_FEATURES_PLAN.md`

## Labels

`enhancement` `feature` `social` `backend` `frontend` `firestore`
